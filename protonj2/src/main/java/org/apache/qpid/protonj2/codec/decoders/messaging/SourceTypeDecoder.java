...
public final class SourceTypeDecoder extends AbstractDescribedListTypeDecoder<Source> {
    ...
    protected Source readType(int count, ProtonBuffer buffer, Decoder decoder, DecoderState state) throws DecodeException {
        ...
        for (int index = 0; index < count; ++index) {
            ...
            switch (index) {
                case 0 -> source.setAddress(decoder.readString(buffer, state));
                case 1 -> source.setDurable(TerminusDurability.valueOf(decoder.readUnsignedInteger(buffer, state, 0)));
                case 2 -> {
                ...
                case 3 -> source.setTimeout(decoder.readUnsignedInteger(buffer, state, UnsignedInteger.ZERO));
                case 4 -> source.setDynamic(decoder.readBoolean(buffer, state, false));
                case 5 -> source.setDynamicNodeProperties(decoder.readMap(buffer, state));
                case 6 -> source.setDistributionMode(decoder.readSymbol(buffer, state));
                case 7 -> source.setFilter(decoder.readMap(buffer, state));
                case 8 -> source.setDefaultOutcome(decoder.readObject(buffer, state, Outcome.class));
                case 9 -> source.setOutcomes(decoder.readMultiple(buffer, state, Symbol.class));
                case 10 -> source.setCapabilities(decoder.readMultiple(buffer, state, Symbol.class));
            }
        }
    ...
    protected Source readType(int count, InputStream stream, StreamDecoder decoder, StreamDecoderState state) throws DecodeException {
        ...
        for (int index = 0; index < count; ++index) {
            ...
            switch (index) {
                case 0 -> source.setAddress(decoder.readString(stream, state));
                case 1 -> source.setDurable(TerminusDurability.valueOf(decoder.readUnsignedInteger(stream, state, 0)));
                case 2 -> {
                    final Symbol expiryPolicy = decoder.readSymbol(stream, state);
                    source.setExpiryPolicy(expiryPolicy == null ? TerminusExpiryPolicy.SESSION_END : TerminusExpiryPolicy.valueOf(expiryPolicy));
                }
                case 3 -> source.setTimeout(decoder.readUnsignedInteger(stream, state, UnsignedInteger.ZERO));
                case 4 -> source.setDynamic(decoder.readBoolean(stream, state, false));
                case 5 -> source.setDynamicNodeProperties(decoder.readMap(stream, state));
                case 6 -> source.setDistributionMode(decoder.readSymbol(stream, state));
                case 7 -> source.setFilter(decoder.readMap(stream, state));
                case 8 -> source.setDefaultOutcome(decoder.readObject(stream, state, Outcome.class));
                case 9 -> source.setOutcomes(decoder.readMultiple(stream, state, Symbol.class));
                case 10 -> source.setCapabilities(decoder.readMultiple(stream, state, Symbol.class));
            }
        }
