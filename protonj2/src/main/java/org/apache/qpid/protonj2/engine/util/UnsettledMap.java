...
public class UnsettledMap<Delivery> implements Map<UnsignedInteger, Delivery> {
    ...
    public boolean containsKey(Object key) {
        if (key instanceof Number number) {
            return containsKey(number.intValue());
        } else {
            return false;
        }
    }
    ...
    private final class UnsettledTackingMapKeys extends AbstractSet<UnsignedInteger> {
        ...
        public boolean remove(Object target) {
            if (target instanceof Number number) {
                return UnsettledMap.this.remove(number.intValue()) != null;
            }
            return false;
        }
    ...
    private record IteratorRemoveResult<Delivery>(UnsettledBucket<Delivery> bucket, int readOffset) {
