...
public final class HeaderTypeDecoder extends AbstractDescribedListTypeDecoder<Header> {
    ...
    protected Header readType(int count, ProtonBuffer buffer, Decoder decoder, DecoderState state) throws DecodeException {
        ...
        for (int index = 0; index < count; ++index) {
            ...
            switch (index) {
                case 0 -> header.setDurable(decoder.readBoolean(buffer, state, false));
                case 1 -> header.setPriority(decoder.readUnsignedByte(buffer, state, Header.DEFAULT_PRIORITY));
                case 2 -> header.setTimeToLive(decoder.readUnsignedInteger(buffer, state, 0l));
                case 3 -> header.setFirstAcquirer(decoder.readBoolean(buffer, state, false));
                case 4 -> header.setDeliveryCount(decoder.readUnsignedInteger(buffer, state, 0l));
            }
        }
    ...
    protected Header readType(int count, InputStream stream, StreamDecoder decoder, StreamDecoderState state) throws DecodeException {
        ...
        for (int index = 0; index < count; ++index) {
            ...
            switch (index) {
                case 0 -> header.setDurable(decoder.readBoolean(stream, state, false));
                case 1 -> header.setPriority(decoder.readUnsignedByte(stream, state, Header.DEFAULT_PRIORITY));
                case 2 -> header.setTimeToLive(decoder.readUnsignedInteger(stream, state, 0l));
                case 3 -> header.setFirstAcquirer(decoder.readBoolean(stream, state, false));
                case 4 -> header.setDeliveryCount(decoder.readUnsignedInteger(stream, state, 0l));
            }
        }
