...
public final class ModifiedTypeDecoder extends AbstractDescribedListTypeDecoder<Modified> {
    ...
    protected Modified readType(int count, ProtonBuffer buffer, Decoder decoder, DecoderState state) throws DecodeException {
        ...
        for (int index = 0; index < count; ++index) {
            switch (index) {
                case 0 -> modified.setDeliveryFailed(decoder.readBoolean(buffer, state, false));
                case 1 -> modified.setUndeliverableHere(decoder.readBoolean(buffer, state, false));
                case 2 -> modified.setMessageAnnotations(decoder.readMap(buffer, state));
            }
        }
    ...
    protected Modified readType(int count, InputStream stream, StreamDecoder decoder, StreamDecoderState state) throws DecodeException {
        ...
        for (int index = 0; index < count; ++index) {
            switch (index) {
                case 0 -> modified.setDeliveryFailed(decoder.readBoolean(stream, state, false));
                case 1 -> modified.setUndeliverableHere(decoder.readBoolean(stream, state, false));
                case 2 -> modified.setMessageAnnotations(decoder.readMap(stream, state));
            }
        }
